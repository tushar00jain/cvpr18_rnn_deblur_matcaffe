...
// SF Usage: caffe_('solver_save', hSolver)
static void solver_save(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
      "Usage: caffe_('solver_save', hSolver)");
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
  //char* snapshot_file = mxArrayToString(prhs[1]);
  //const int solver_history_num = 1;
  //const char* solver_history[solver_history_num] = { "history" };
  //mxArray* mx_solver_history = mxCreateStructMatrix(1,1, solver_history_num,
    //solver_history);
  //mxSetField(mx_solver_history, 0, "history",
    //ptr_to_handle<Net<float> >(solver->history()));
  //plhs[0] = mx_solver_history;
  solver->Snapshot();
...
// SF: Usage: caffe_('solver_set_iter', hsolver, iter_)
static void solver_set_iter(MEX_ARGS) {
  mxCHECK(nrhs == 2 && mxIsStruct(prhs[0]) && mxIsDouble(prhs[1]),
      "Usage: caffe_('solver_set_iter', hSolver, iter_)");
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
  int iters = mxGetScalar(prhs[1]);
  ostringstream msg;
  msg << "setting iteration " << iters;
  solver->set_iter(iters);
  //plhs[0] = mxCreateDoubleScalar(solver->iter());
}
...
// SF: Usage: caffe_('solver_update', hSolver, solver_type)
static void solver_update(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
      "Usage: caffe_('solver_step', hSolver)");
	//char* solver_type = mxArrayToString(prhs[1]);
	Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
	  //if (strcmp(solver_type, "SGD") == 0) {
	//	SGDSolver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
	  //} else if (strcmp(solver_type, "Adam") == 0) {
	//	AdamSolver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
	  //} else {
	    //mxERROR("please develop the specific solver based on src/caffe/solvers.");
	  //}
  //int iters = mxGetScalar(prhs[1]);
  solver->ApplyUpdate();
}
...
// Usage: caffe_('net_forwardfromto', hNet,start,end)
static void net_forwardfromto(MEX_ARGS) {
  mxCHECK(nrhs == 3 && mxIsStruct(prhs[0]),"Usage: caffe_('net_forwardfromto', hNet,start,end)");
  Net<float>* net = handle_to_ptr<Net<float> >(prhs[0]);
  int s ,e;
  s = mxGetScalar(prhs[1]);
  e = mxGetScalar(prhs[2]);
  net->ForwardFromTo(s,e);
}

// Usage: caffe_('net_backwardfromto', hNet,start,end)
static void net_backwardfromto(MEX_ARGS) {
  mxCHECK(nrhs == 3 && mxIsStruct(prhs[0]),"Usage: caffe_('net_forwardfromto', hNet,start,end)");
  Net<float>* net = handle_to_ptr<Net<float> >(prhs[0]);
  int s ,e;
  s = mxGetScalar(prhs[1]);
  e = mxGetScalar(prhs[2]);
  net->BackwardFromTo(s,e);
}
...
static void get_weights(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
      "Usage: caffe_('get_weight', hNet)");
  Net<float>* net_ = handle_to_ptr<Net<float> >(prhs[0]);
...
static void set_weights(MEX_ARGS) {
  mxCHECK(nrhs == 2 && mxIsStruct(prhs[0]) && mxIsStruct(prhs[1]),
      "Usage: caffe_('set_weight', hNet, weight)");
  Net<float>* net_ = handle_to_ptr<Net<float> >(prhs[0]);
  const mxArray* const mx_weights = prhs[1];
      LOG(INFO)<<"set weights test";
    ...
    for (unsigned int i = 0; i < layers.size(); ++i) {
     ...
     if (strcmp(layer_names[i].c_str(),c_layer_names) == 0) {
      ...
      if (layer_blobs.size() == 0) {
        ...
        if (static_cast<unsigned int>(mxGetDimensions(weights)[0]) != layer_blobs.size()) {
            mxERROR("Num of cells don't match layer_blobs.size");
        }
        LOG(INFO) << "layer_blobs.size() = " << layer_blobs.size();
        for (unsigned int j = 0; j < layer_blobs.size(); ++j) {
           // internally data is stored as (width, height, channels, num)
           // where width is the fastest dimension
           const mxArray* const elem = mxGetCell(weights, j);
           mwSize dims[4] = {layer_blobs[j]->width(), layer_blobs[j]->height(),
            layer_blobs[j]->channels(), layer_blobs[j]->num()};
           DLOG(INFO) << dims[0] << " " << dims[1] << " " << dims[2] << " " << dims[3];
           if (layer_blobs[j]->count() != mxGetNumberOfElements(elem)) {
              mxERROR("Numel of weights don't match count of layer_blob");
           }
...
// SF: get all blobs in one pass
static void get_all_data(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
      "Usage: caffe_('get_all_data', hNet)");
  Net<float>* net_ = handle_to_ptr<Net<float> >(prhs[0]);
...
static void get_all_diff(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
      "Usage: caffe_('get_all_diff', hNet)");
  Net<float>* net_ = handle_to_ptr<Net<float> >(prhs[0]);
  const vector<shared_ptr<Blob<float> > >& blobs = net_->blobs();
  const vector<string>& blob_names = net_->blob_names();
...
// SF: loading pre-trained model
// Usage: caffe_('net_share', hNet, hNet_pre)
static void net_share(MEX_ARGS) {
  mxCHECK(nrhs == 2 && mxIsChar(prhs[0]) && mxIsChar(prhs[1]),
      "Usage: caffe_('net_share', hNet, hNet_pre)");
  Net<float>* net = handle_to_ptr<Net<float> >(prhs[0]);
  Net<float>* net_pre = handle_to_ptr<Net<float> >(prhs[1]);
  net->ShareTrainedLayersWith(net_pre);
}
...
static handler_registry handlers[] = {
  ...
  { "net_forwardfromto",  net_forwardfromto},
  { "net_backward",       net_backward    },
  { "net_backwardfromto", net_backwardfromto},
